...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("명화 선호도 투표 (20200808 강석규)");
            ...
            image[index].setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (voteCount[index] >= 5) {
                        Toast.makeText(getApplicationContext(), "투표 수 한계에 도달했습니다.", Toast.LENGTH_SHORT).show();
                    } else {
                        voteCount[index]++;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            int sum = data.getIntExtra("Max", 0);
            for (int i = 0; i < 9; i++)
                voteCount[i] = 0;
            Toast.makeText(getApplicationContext(), "점수가 초기화되었습니다.", Toast.LENGTH_SHORT).show();
        }
    }
}